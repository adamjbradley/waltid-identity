...
const { createApp, config, jwkThumbprint, extractCnfThumbprint } = require('../server');
...
// ============================================================
// Cnf-key wallet-to-session binding
// ============================================================

describe('jwkThumbprint (unit)', () => {
  // RFC 7638 §3.1 canonical RSA vector. If the helper disagrees with
  // this, it's wrong — this is the textbook reference value.
  ...
  it('matches the RFC 7638 RSA reference vector', () => {
    expect(jwkThumbprint(RFC7638_RSA_JWK)).toBe(
      'NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs',
    );
  });

  it('computes a stable thumbprint for an EC key', () => {
    const jwk = {
      kty: 'EC',
      crv: 'P-256',
    ...
    const t2 = jwkThumbprint(Object.assign({}, jwk, { use: 'sig', kid: 'ignored' }));
    expect(typeof t1).toBe('string');
    expect(t1.length).toBeGreaterThan(0);
    // Extra members (use, kid) must not affect the thumbprint.
    expect(t2).toBe(t1);
  });

  it('handles OKP keys (crv, kty, x only)', () => {
    const jwk = {
      kty: 'OKP',
      crv: 'Ed25519',
      x: '11qYAYKxCrfVS_7TyWQHOg7hcvPapiMlrwIaaPcHURo',
    };
    expect(typeof jwkThumbprint(jwk)).toBe('string');
  });

  it('returns null on malformed input', () => {
    expect(jwkThumbprint(null)).toBeNull();
    expect(jwkThumbprint(undefined)).toBeNull();
    expect(jwkThumbprint({})).toBeNull();
    expect(jwkThumbprint({ kty: 'EC', crv: 'P-256', x: 'x' /* no y */ })).toBeNull();
    expect(jwkThumbprint({ kty: 'RSA', e: 'AQAB' /* no n */ })).toBeNull();
    expect(jwkThumbprint({ kty: 'OKP', x: 'x' /* no crv */ })).toBeNull();
    expect(jwkThumbprint({ kty: 'oct', k: 'secret' })).toBeNull();
  });
});

describe('extractCnfThumbprint (unit)', () => {
  it('walks presentedCredentials and returns the first cnf.jwk thumbprint', () => {
    const presented = {
      pid_0: [{
        credentialData: {
          given_name: 'Alice',
    ...
    const t = extractCnfThumbprint(presented);
    expect(typeof t).toBe('string');
    expect(t).toBe(jwkThumbprint(presented.pid_0[0].credentialData.cnf.jwk));
  });

  it('returns null when no credential carries cnf', () => {
    expect(extractCnfThumbprint({
      pid_0: [{ credentialData: { given_name: 'A' } }],
    })).toBeNull();
  });

  it('returns null on empty or invalid input', () => {
    expect(extractCnfThumbprint(null)).toBeNull();
    expect(extractCnfThumbprint(undefined)).toBeNull();
    expect(extractCnfThumbprint({})).toBeNull();
    expect(extractCnfThumbprint({ pid_0: 'not-an-array' })).toBeNull();
  });
});

describe('Cnf-binding on age-check', () => {
  // The webhook captures the presentation's cnf thumbprint; the status
  // poll compares it against req.session.cnfThumbprint (stashed at OIDC
  // callback time from the id_token's cnf_jkt claim).
  const WALLET_A_JWK = {
    kty: 'EC', crv: 'P-256',
    x: 'f83OJ3D2xF1Bg8vub9tLe1gHMzV76e8Tus9uPHvRVEU',
    y: 'x_FEzRu9m36HLN_tue659LNpXW6pCyStikYjKIWI5a0',
  ...
  const WALLET_B_JWK = {
    kty: 'EC', crv: 'P-256',
    x: 'bzDmlUytJGdT2zCnc0qBfkAopRPhOEM1jPkrUmBXOG8',
    y: 'jXgZ5mVXpQRkElJFJ4-Yf7XnQWGJKuxBf7vUOgZaFy0',
  };
  ...
  beforeEach(() => {
    const tmp = path.join(os.tmpdir(), 'rp-cnf-age-' + Date.now() + '-' + Math.random().toString(36).slice(2) + '.json');
    tmpFiles.push(tmp);
    process.env.USER_STORE_FILE = tmp;
    app = createApp();
    agent = request.agent(app);
  ...
  async function seedSessionWithCnf(token, secret, cnfThumbprint) {
    const sessionPayload = {
      ageCheck: { sessionId: 's-' + token, token, webhookSecret: secret },
    };
    if (cnfThumbprint) sessionPayload.cnfThumbprint = cnfThumbprint;
    await agent.post('/_test/session').send(sessionPayload).expect(200);
    await agent.post('/_test/age-check/register').send({ token, webhookSecret: secret }).expect(200);
  }

  function successfulAgeWebhook(jwk, sessionId) {
    const credentialData = { age_over_21: true };
    if (jwk) credentialData.cnf = { jwk };
    return {
      event: 'policy_results_available',
      session: {
        id: sessionId,
  ...
  it('matching cnf -> verified:true, userStore updated', async () => {
    // adminUpdate requires an existing profile in the store (no upsert
    // semantics), so seed one before the kickoff the way the "promotes
    // age flags" test does — mirrors the real flow where the OIDC
    // callback's userStore.upsert has already run by the time a shopper
    // hits the age-check gate.
    const { UserStore } = require('../userStore');
    const seedStore = new UserStore(process.env.USER_STORE_FILE);
    seedStore.upsert({ sub: 'sub-match', provider: 'authop' });
    app = createApp();
    agent = request.agent(app);

    const expectedJkt = jwkThumbprint(WALLET_A_JWK);
    await agent.post('/_test/session').send({
      user: { sub: 'sub-match', provider: 'authop' },
      provider: 'authop',
      cnfThumbprint: expectedJkt,
      ageCheck: { sessionId: 's-match', token: 'tok-match', webhookSecret: 'secret-match' },
    }).expect(200);
    await agent.post('/_test/age-check/register').send({ token: 'tok-match', webhookSecret: 'secret-match' }).expect(200);

    await request(app).post('/api/age-check/webhook/tok-match')
      .set('Authorization', 'Bearer secret-match')
      .send(successfulAgeWebhook(WALLET_A_JWK, 's-match'))
      .expect(200);

    const status = await agent.get('/api/age-check/status').expect(200);
    expect(status.body).toEqual({ verified: true });

    // userStore was promoted via adminUpdate.
    const persisted = new UserStore(process.env.USER_STORE_FILE);
    const saved = persisted.get('sub-match');
    expect(saved && saved.age_over_21).toBe(true);
    expect(saved && saved.age_over_18).toBe(true);
  });

  it('mismatched cnf -> verified:false, mismatch cnf_mismatch, userStore unchanged', async () => {
    const expectedJkt = jwkThumbprint(WALLET_A_JWK);
    await agent.post('/_test/session').send({
      user: { sub: 'sub-mismatch', provider: 'authop' },
      provider: 'authop',
      cnfThumbprint: expectedJkt,
      ageCheck: { sessionId: 's-mis', token: 'tok-mis', webhookSecret: 'secret-mis' },
    }).expect(200);
    await agent.post('/_test/age-check/register').send({ token: 'tok-mis', webhookSecret: 'secret-mis' }).expect(200);

    // Wallet B presents — different thumbprint.
    await request(app).post('/api/age-check/webhook/tok-mis')
      .set('Authorization', 'Bearer secret-mis')
      .send(successfulAgeWebhook(WALLET_B_JWK, 's-mis'))
      .expect(200);

    const status = await agent.get('/api/age-check/status').expect(200);
    expect(status.body).toEqual({ verified: false, mismatch: 'cnf_mismatch' });

    // userStore NOT updated.
    await agent.post('/_test/session').send({
      user: { sub: 'sub-mismatch', provider: 'keycloak', email: 'adam_j_bradley@yahoo.com' },
      provider: 'keycloak',
    }).expect(200);
    const users = await agent.get('/api/admin/users').expect(200);
    const saved = users.body.users.find((u) => u.sub === 'sub-mismatch');
    // Profile may not exist at all (no upsert from age-check flow), OR
    // if a prior seed created it, age_over_21 MUST NOT be true.
    if (saved) expect(saved.age_over_21).not.toBe(true);
  });

  it('anonymous session (no cnfThumbprint) skips the binding check', async () => {
    await seedSessionWithCnf('tok-anon-cnf', 'secret-anon-cnf', null);
    // Any wallet key — no binding exists.
    await request(app).post('/api/age-check/webhook/tok-anon-cnf')
      .set('Authorization', 'Bearer secret-anon-cnf')
      .send(successfulAgeWebhook(WALLET_B_JWK, 's-tok-anon-cnf'))
      .expect(200);
    const status = await agent.get('/api/age-check/status').expect(200);
    expect(status.body).toEqual({ verified: true });
  });
...
describe('Cnf-binding on checkout', () => {
  const WALLET_A_JWK = {
    kty: 'EC', crv: 'P-256',
    x: 'f83OJ3D2xF1Bg8vub9tLe1gHMzV76e8Tus9uPHvRVEU',
    y: 'x_FEzRu9m36HLN_tue659LNpXW6pCyStikYjKIWI5a0',
  ...
  beforeEach(() => {
    const tmp = path.join(os.tmpdir(), 'rp-cnf-ck-' + Date.now() + '-' + Math.random().toString(36).slice(2) + '.json');
    tmpFiles.push(tmp);
    process.env.USER_STORE_FILE = tmp;
    app = createApp();
    agent = request.agent(app);
    global.fetch = jest.fn(async (url) => {
      if (typeof url === 'string' && url.includes('/verification-session/create')) {
        return {
          ok: true, status: 200,
          json: async () => ({
            sessionId: 'ck-cnf-sess',
            bootstrapAuthorizationRequestUrl: 'openid4vp://authorize?request_uri=ck_cnf',
            fullAuthorizationRequestUrl: 'openid4vp://authorize?request_uri=ck_cnf_full',
          }),
        };
      }
      throw new Error('unexpected fetch ' + url);
  ...
  async function seedAndKickoff(cnfThumbprint) {
    const sessionPayload = {
      user: { sub: 'sub-ck-cnf', provider: 'authop', age_over_21: true },
      provider: 'authop',
      ageVerified: true,
    };
    if (cnfThumbprint) sessionPayload.cnfThumbprint = cnfThumbprint;
    ...
    const token = webhookUrl.split('/').pop();
    const bearer = reqBody.core_flow.notifications.webhook.bearer_token;
    return { orderId: res.body.orderId, token, bearer };
  }

  function pwaWebhookBody(jwk) {
    const credentialData = { panLastFour: '4242', scheme: 'Visa' };
    if (jwk) credentialData.cnf = { jwk };
    return {
      event: 'policy_results_available',
      session: {
        status: 'SUCCESSFUL',
        presentedCredentials: { pwa_0: [{ credentialData }] },
      },
    };
  }

  it('matching cnf -> order recorded, cart cleared', async () => {
    const jkt = jwkThumbprint(WALLET_A_JWK);
    const { orderId, token, bearer } = await seedAndKickoff(jkt);
    await request(app).post('/api/checkout/webhook/' + token)
      .set('Authorization', 'Bearer ' + bearer)
      .send(pwaWebhookBody(WALLET_A_JWK))
      .expect(200);
    const status = await agent.get('/api/checkout/status').expect(200);
    expect(status.body).toEqual({ status: 'completed', orderId });
    const cart = await agent.get('/api/cart').expect(200);
    expect(cart.body.count).toBe(0);
  });

  it('mismatched cnf -> declined cnf_mismatch, cart NOT cleared', async () => {
    const jkt = jwkThumbprint(WALLET_A_JWK);
    const { token, bearer } = await seedAndKickoff(jkt);
    await request(app).post('/api/checkout/webhook/' + token)
      .set('Authorization', 'Bearer ' + bearer)
      .send(pwaWebhookBody(WALLET_B_JWK))
      .expect(200);
    const status = await agent.get('/api/checkout/status').expect(200);
    expect(status.body).toEqual({ status: 'declined', reason: 'cnf_mismatch' });
    // Cart still has the item.
    const cart = await agent.get('/api/cart').expect(200);
    expect(cart.body.count).toBe(1);
  });

  it('anonymous (no cnfThumbprint) -> binding skipped, order recorded', async () => {
    const { orderId, token, bearer } = await seedAndKickoff(null);
    // Even a wallet with no cnf disclosure at all passes.
    await request(app).post('/api/checkout/webhook/' + token)
      .set('Authorization', 'Bearer ' + bearer)
      .send(pwaWebhookBody(null))
      .expect(200);
    const status = await agent.get('/api/checkout/status').expect(200);
    expect(status.body).toEqual({ status: 'completed', orderId });
  });
