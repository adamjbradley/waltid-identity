...
const COUNTRY_FLAGS: Record<string, string> = {
  AT: '\u{1F1E6}\u{1F1F9}', BE: '\u{1F1E7}\u{1F1EA}', BG: '\u{1F1E7}\u{1F1EC}', HR: '\u{1F1ED}\u{1F1F7}',
  CY: '\u{1F1E8}\u{1F1FE}', CZ: '\u{1F1E8}\u{1F1FF}', DK: '\u{1F1E9}\u{1F1F0}', EE: '\u{1F1EA}\u{1F1EA}',
  FI: '\u{1F1EB}\u{1F1EE}', FR: '\u{1F1EB}\u{1F1F7}', DE: '\u{1F1E9}\u{1F1EA}', EL: '\u{1F1EC}\u{1F1F7}',
  GR: '\u{1F1EC}\u{1F1F7}', HU: '\u{1F1ED}\u{1F1FA}', IE: '\u{1F1EE}\u{1F1EA}', IT: '\u{1F1EE}\u{1F1F9}',
  LV: '\u{1F1F1}\u{1F1FB}', LT: '\u{1F1F1}\u{1F1F9}', LU: '\u{1F1F1}\u{1F1FA}', MT: '\u{1F1F2}\u{1F1F9}',
  NL: '\u{1F1F3}\u{1F1F1}', PL: '\u{1F1F5}\u{1F1F1}', PT: '\u{1F1F5}\u{1F1F9}', RO: '\u{1F1F7}\u{1F1F4}',
  SK: '\u{1F1F8}\u{1F1F0}', SI: '\u{1F1F8}\u{1F1EE}', ES: '\u{1F1EA}\u{1F1F8}', SE: '\u{1F1F8}\u{1F1EA}',
  EU: '\u{1F1EA}\u{1F1FA}', NO: '\u{1F1F3}\u{1F1F4}', IS: '\u{1F1EE}\u{1F1F8}', LI: '\u{1F1F1}\u{1F1EE}',
  UK: '\u{1F1EC}\u{1F1E7}', TR: '\u{1F1F9}\u{1F1F7}', RS: '\u{1F1F7}\u{1F1F8}', ME: '\u{1F1F2}\u{1F1EA}',
  MK: '\u{1F1F2}\u{1F1F0}', AL: '\u{1F1E6}\u{1F1F1}', CH: '\u{1F1E8}\u{1F1ED}',
}
...
export default function TrustConfig() {
  ...
  const [activeTab, setActiveTab] = useState<'status' | 'trust-lists'>('status')
  ...
  const [countryDetail, setCountryDetail] = useState<Record<string, CountryTslDetail>>({})
  ...
  const handleToggleSource = async (sourceName: string, currentEnabled: boolean) => {
    try {
      ...
      await axios.put(`${apiBase}/admin/trust/${endpoint}`, { enabled: !currentEnabled })
      await fetchStatus()
    } catch (e: any) {
  ...
  const handleTestValidation = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${apiBase}/admin/trust/test`, { params })
  ...
  const fetchLotl = useCallback(async () => {
    ...
    try {
      ...
      if (e.response?.status === 404) {
        setLotlError('LOTL data not available. Try refreshing trust lists from the Status tab.')
      } else if (e.response?.status === 503) {
        setLotlError('Trust lists feature is not enabled.')
      } else {
        setLotlError(e.response?.data?.message || e.message || 'Failed to fetch LOTL data')
      }
    } finally {
  ...
  const handleCountryClick = async (country: string) => {
    ...
    try {
      const response = await axios.get(`${apiBase}/admin/trust/lotl/${country}`)
              ...
              <>
                <div className="space-y-4 mb-8">
                  ...
                  {Object.entries(status.sources).map(([sourceKey, source]) => {
                    ...
                    return (
                      <div key={sourceKey} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                          ...
                          <div className="flex-1">
                            ...
                            {testResult.trusted && (
                              <div className="mt-2 text-sm text-gray-700 space-y-1">
                                {testResult.source && <p>Source: <span className="font-medium">{testResult.source}</span></p>}
                                {testResult.provider && <p>Provider: <span className="font-medium">{testResult.provider}</span></p>}
                                {testResult.country && <p>Country: <span className="font-medium">{testResult.country}</span></p>}
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    {searchResults.providers.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {searchResults.providers.map((provider, idx) => (
                          <SearchResultCard key={`${provider.name}-${idx}`} provider={provider} />
                        ))}
                      </div>
                    )}
                  </div>
                      ...
                      <>
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-lg p-5 mb-6">
                          <div className="flex items-start justify-between">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <h3 className="font-semibold text-gray-900">EU List of Trusted Lists</h3>
                              </div>
                              <p className="text-sm text-gray-600">{lotl.schemeOperatorName}</p>
                            </div>
                            <div className="text-right text-xs text-gray-500 space-y-1">
                              {lotl.sequenceNumber && <p>Seq #{lotl.sequenceNumber}</p>}
                          ...
                          <div className="flex gap-4 mt-3 text-sm">
                            <span className="text-gray-600">
                              <span className="font-medium text-gray-900">{lotl.memberStates.length}</span> member states
                            </span>
                            <span className="text-gray-600">
                              <span className="font-medium text-gray-900">
                                {lotl.memberStates.reduce((sum, ms) => sum + ms.providerCount, 0)}
                              </span> providers
                            </span>
                            <span className="text-gray-600">
                              <span className="font-medium text-gray-900">
                                {lotl.memberStates.reduce((sum, ms) => sum + ms.serviceCount, 0)}
                              </span> services
                            </span>
                          </div>
                        </div>
                          ...
                          {lotl.memberStates
                            ...
                            >
                              <div className="flex items-center justify-between mb-1.5">
                                <div className="flex items-center gap-2">
                                  <span className="text-lg">{getFlag(ms.country)}</span>
                                  <span className="font-medium text-sm text-gray-900">{ms.country}</span>
                                </div>
                                <div className={`w-2 h-2 rounded-full ${ms.healthy ? 'bg-emerald-500' : 'bg-red-400'}`} />
                              </div>
                              <div className="text-xs text-gray-500">
                                {ms.providerCount} provider{ms.providerCount !== 1 ? 's' : ''} &middot; {ms.serviceCount} service{ms.serviceCount !== 1 ? 's' : ''}
                              </div>
                            </button>
                          ))}
                        </div>
                        ...
                        {expandedCountry && (
                          <div className="border border-blue-200 rounded-lg bg-blue-50/30 p-5 mt-2 mb-4">
                            {countryLoading === expandedCountry ? (
                              ...
                              <CountryDetailPanel detail={countryDetail[expandedCountry]} formatDate={formatDate} />
                            ) : (
                              <p className="text-sm text-gray-500 text-center py-4">Failed to load details for {expandedCountry}</p>
                            )}
                          </div>
                        )}
                      </>
...
function CountryDetailPanel({ detail, formatDate }: { detail: CountryTslDetail, formatDate: (d?: string) => string }) {
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-gray-900">{detail.schemeTerritory} Trusted List</h3>
          </div>
          <p className="text-sm text-gray-600 mt-0.5">{detail.schemeOperatorName}</p>
        </div>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm text-gray-900 truncate">{provider.name}</span>
                </div>
                {provider.tradeName && provider.tradeName !== provider.name && (
                  <p className="text-xs text-gray-500 ml-6 truncate">{provider.tradeName}</p>
                )}
              </div>
                ...
                <table className="w-full text-sm mt-2">
                  ...
                  <tbody>
                    {provider.services.map((svc, svcIdx) => (
                      <tr key={svcIdx} className="border-t border-gray-50">
                        <td className="py-2 pr-3 text-gray-800 max-w-[200px] truncate" title={svc.serviceName}>
                          {svc.serviceName}
                        </td>
                        <td className="py-2 pr-3">
                          <span className="inline-flex items-center gap-1">
                            <span className="text-gray-600">{svc.serviceTypeLabel}</span>
                            {svc.isQualified && (
                              <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium">Q</span>
                            )}
                          </span>
                        </td>
                        <td className="py-2 pr-3">
                        ...
                        <td className="py-2 text-xs text-gray-500">{formatDate(svc.statusStartingTime)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {provider.country && <span className="text-base">{getFlag(provider.country)}</span>}
            <span className="font-medium text-sm text-gray-900 truncate">{provider.name}</span>
          </div>
          {provider.tradeName && provider.tradeName !== provider.name && (
            ...
            {provider.services.map((svc, idx) => (
              <div key={idx} className="flex items-center justify-between text-sm py-1">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span className="text-gray-800 truncate">{svc.serviceName}</span>
                  {svc.isQualified && (
                    <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-medium flex-shrink-0">Q</span>
                  )}
                </div>
                <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                  <span className="text-xs text-gray-500">{svc.serviceTypeLabel}</span>
                  <StatusBadge status={svc.status} />
                </div>
              </div>
            ))}
